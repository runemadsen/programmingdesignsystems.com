...
// and renders it in instance mode. Only works because I know what code
// I'm writing.
    ...
    function p5tag(context, tag, input) {
      ...
      for (var i = 0; i < replacements.length; i++) {
        ...
        if (typeof replacements[i] == "string") {
          var escaped = replacements[i].replace("(", "\\(").replace(")", "\\)");
        ...
        // if replacement is array with before/after
          ...
          code = code.replace(
            new RegExp(replacements[i][0].replace("(", "\\("), "g"),
            replacements[i][1]
          );
        }
